...
<body>
	...
	<script type="text/javascript">
		function page(page){
			var page = page;
			location.href
			="?page="+page+"&enterprise_category=<%=enterprise_category%>&local_category=<%=local_category%>&gender=<%=gen%>&edu=<%=edu%>&day=<%=day%>&search=<%=search%>";
						...
						<div class="collapse show" id="collapseCardExample">
							<div class="card-body">
								<table>
									...
									<th>작성일자</th>
									<tr>
										<form action="<%=request.getContextPath()%>/admin/board/recruit"
											method="get" name="keyword" id="keyword">
											<td><select id="enterprise_category"
											...
											<td><select id="local_category" name="local_category">
											...
											<td><select name="day" style="width: 150px;">
													<option value="">--선택하십시오--</option>
							...
							<div>
								...
								<div class="card card-stats"
									...
									<div class="card-header card-header-warning card-header-icon">
										...
										<h3 class="card-title" id="todaycount" name="todaycount"><%=request.getAttribute("todaycount")%><small>개</small>
										</h3>
									</div>
								...
								<div class="card card-stats"
									...
									<div class="card-header card-header-warning card-header-icon">
										...
										<h3 class="card-title" id="yesterdaycount"
											name="yesterdaycount"><%=request.getAttribute("yesterdaycount")%><small>개</small>
										</h3>
									</div>
								</div>
							</div>
						...
						<div class="card-body">
							...
							<div class="table-responsive">
								<table class="table" id="res">
									<thead>
										<button type="submit" onclick="deleteRecruits()"
											class="btn py-1 px-1 btn-danger" style="float: right;">삭제</button>
										<tr>
									...
									<tbody>
										...
										%>
										<tr>
											<td><input type='checkbox' name="boardrecruit_id"
												class='memberChk' onclick='OnOffMemberAllClickBtn()'
												value="<%=boardrecruit_id%>"></td>
											<td><%=boardrecruit_id%></td>
											<td><%=boardrecruit_date%></td>
											<td><a
												href="<%=request.getContextPath()%>/recruit/content?board_recruit_id=<%=boardrecruit_id%>"><%=boardrecruit_title%></a></td>
											<td><%=enterprise_id%></td>
											<td><a
												href="<%=request.getContextPath()%>/admin/board/volunteerlist?board_recruit_id=<%=boardrecruit_id%>"
												class="btn py-1 px-1 btn-primary">지원자보기</a>
											<td><%=category%></td>
											<td><%=place%></td>
											<td><%=gender%></td>
											<td><%=education%></td>
